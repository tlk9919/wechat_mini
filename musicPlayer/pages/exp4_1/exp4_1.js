  ...
  Page({
    ...
    onReady: function () {
      this.audioBam = wx.getBackgroundAudioManager() // 获取音频管理器
      // 默认选择第1曲
      this.setMusic(0)
      // 播放失败检测
      ...
      this.audioBam.onTimeUpdate(() => {
        var currentTime = parseInt(this.audioBam.currentTime)// 获取当前播放时间
        if (!this.sliderChangeLock && currentTime !== updateTime) {  // 将更新频率限制在1秒1次
          updateTime = currentTime
          //更新页面的播放进度、当前播放时间和音频的总时长。
          this.setData({
    ...
    sliderChanging: function (e) {
      ...
      this.setData({
        'play.currentTime': formatTime(second),// 更新当前时间显示
      })
    },
    // 设置当前播放的曲目
